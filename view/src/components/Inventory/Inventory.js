...
class Inventory extends React.Component {
  ...
  async componentDidMount() {
    await axios.get('/flora/inventory/list/retrieve').then(res => {
      ...
      autocomplete(document.getElementById("myInput"), res.data);

      function autocomplete(inp) {

        var arr = [];
        for (var i = 0; i < res.data.length; i++) {
          var newArr = res.data[i]
          for (var x = 0; x < newArr[2].length; x++) {

            arr.push(newArr[2][x][1])
          }
        };
  ...
  render() {
    return (
      <div className="container-fluid inventory">
        ...
        <div className="row m-5">
          <div className="col">
            <div className="input-group autocomplete input-group-lg">
              <div className="input-group-prepend">
                <span className="input-group-text bg-success text-white" id="inputGroup-sizing-lg">Search Flora</span>
              </div>
              <input type="text" id="myInput" className="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-lg"></input>
            </div>
          </div>
        </div>
